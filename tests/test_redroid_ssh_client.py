...
def test_exec_builds_ssh_command_with_port_user_and_key(monkeypatch, tmp_path):
    ...
    client = RedroidSSHClient(host="<host-agent-node>", port=22, user="dd", key_path=str(key_path))
...
def test_exec_returns_stderr_and_nonzero_code(monkeypatch):
    ...
    client = RedroidSSHClient(host="<host-agent-node>", user="dd")
...
def test_scp_fetch_builds_copy_command(monkeypatch, tmp_path):
    ...
    client = RedroidSSHClient(host="<host-agent-node>", port=2222, user="dd", key_path=str(key_path))
    ...
    assert calls[0] == [
        ...
        "-o",
        "StrictHostKeyChecking=no",
        "dd@<host-agent-node>:/tmp/conn.log",
        str(local_path),
    ]
...
def test_exec_supports_ssh_password_via_sshpass(monkeypatch):
    ...
    client = RedroidSSHClient(host="<host-agent-node>", user="user", password="secret")
